...
fn read_catalog(file: &mut File) -> std::io::Result<Vec<DirEntry>> {
...
fn main() -> std::io::Result<()> {
    ...
    if args.len() < 3 {
        eprintln!("Usage: {} <diskimage> <user:filename.filetype>", args[0]);
        return Ok(());
    }
...
// Leta upp filen
    if let Some(file_entry) = files.iter().find(|f| {
        ...
        let mut out = File::create("test.bin")?;
        ...
        eprintln!("File {} not found on disk {}", source_file, diskimage);
    }
